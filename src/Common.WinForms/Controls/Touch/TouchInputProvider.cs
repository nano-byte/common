...
/// <summary>
/// Translates Windows pointer input on a <see cref="Control"/> into high-level touch gestures (tap and manipulation).
/// </summary>
public sealed partial class TouchInputProvider : IDisposable
{
    ...
    {
        get => _doubleTapEnabled;
        set
        {
            if (_doubleTapEnabled == value) return;
            _doubleTapEnabled = value;
            if (_enabled) ConfigureInteractions();
        }
    }
    ...
    public TouchInputProvider(Control control)
    {
    ...
    private void ConfigureInteractions()
    {
        var configuration = new NativeMethods.InteractionContextConfiguration[]
        {
            ...
            new()
            {
                interactionId = NativeMethods.InteractionId.Tap,
                enable = _doubleTapEnabled ? NativeMethods.ConfigTap | NativeMethods.ConfigTapDouble : NativeMethods.ConfigTap
            }
        };
    ...
    private void OnOutput(IntPtr clientData, IntPtr outputPtr)
    {
        ...
        {
            case NativeMethods.InteractionId.Tap:
                Tapped?.Invoke(_control, new TappedEventArgs(origin, (int)output.tapCount));
                break;
